...
| Champ | Description |
|--- |--- |
| [!UICONTROL Magento Price Source] | Détermine la source de prix utilisée lors de la création de vos listes Amazon. La valeur par défaut est `Price`. Si vous choisissez un attribut différent, tel que `Amazon Price` ou `Special Price`, la valeur définie pour l’attribut est utilisée pour votre liste Amazon. Cependant, si l’attribut sélectionné n’est pas défini, `Price` est utilisé. |
...
| [!UICONTROL Strike Through Price (MSRP)] | L’attribut [!DNL Commerce] qui représente le prix MSRP. Si le prix de votre offre Amazon est inférieur au MSRP, il affiche une répercussion sur le prix MSRP et le prix de la vente. Ce paramètre est également utilisé pour calculer le montant et le pourcentage &quot;Vous économisez&quot;, mais cette fonctionnalité ne s’applique qu’aux listes qui ont obtenu la position [Buy Box](./buy-box-competitor-pricing.md). |
| [!UICONTROL Apply Value Added Tax (VAT)] | La TVA est utilisée par les vendeurs dans l&#39;Union Européenne.<br><br>Choisissez  `Disabled` si vous ne souhaitez pas que la TVA soit ajoutée aux prix listés.<br><br>Sélectionnez  `Enabled` et saisissez le Pourcentage de TVA afin d&#39;appliquer la TVA aux prix de vos listes. |
| [!UICONTROL VAT Percentage] | Définissez le pourcentage à utiliser pour calculer le montant de la TVA à ajouter au prix de la mise en vente de vos listes Amazon. <br><br>Si vous saisissez  `5`, une TVA de 5 % sera appliquée au prix de vente final une fois toutes les règles de prix appliquées. La taxe TVA ne s&#39;applique pas au prix final pour les inscriptions utilisées dans une règle de prix intelligente, sauf si le [plancher](./floor-price.md) ou [plafond](./optional-ceiling-price.md) est atteint. |
| [!UICONTROL Amazon Product Tax Code (PTC)] | (Apparaît pour les magasins britanniques uniquement) Détermine si le canal de vente Amazon envoie des informations de code de taxe sur les produits à votre compte [!DNL Amazon Seller Central]. <br><br>Sélectionnez  **Ne pas gérer** les impôts si vous utilisez un service de calcul des impôts tiers ou si tous vos calculs d&#39;impôts sont déjà configurés dans votre  [!DNL Amazon Seller Central] compte. Lorsque cette option est définie, le canal de vente Amazon n’envoie aucune information de code de taxe sur les produits à votre compte [!DNL Amazon Seller Central].<br><br>Sélectionnez  **Définir le** code de taxe par défaut si vous disposez d’un code de taxe universelle sur les produits que vous souhaitez utiliser pour tous vos produits.<br><br>Voir  [Codes d’impôt sur les produits Amazon](https://sellercentral.amazon.com/gp/help/help.html?itemID=G200794510&amp;language=en_US){target=&quot;_blank&quot;}. |
| [!UICONTROL Default PTC] | S’affiche uniquement lorsque **Code de taxe sur les produits Amazon (PTC)** est défini sur `Set Default PTC`. Saisissez le code PTC par défaut à utiliser pour toutes les listes Amazon éligibles. Si votre PTC par défaut est défini dans votre compte [!DNL Amazon Seller Central], laissez ce champ vide. <br><br>Les modifications apportées à ce champ n’ont aucune incidence sur les listes existantes. La liste doit être [terminée](./end-listings-manually.md) et une nouvelle liste doit être créée pour que la modification soit prise en compte. |
| [!UICONTROL Currency Conversion] | Permet à votre devise [!DNL Commerce] storefront par défaut de se convertir précisément en votre devise Amazon par défaut pour publier les prix de vos listes dans la devise appropriée. La conversion de devise est toujours basée sur votre devise par défaut [!DNL Commerce].<br><br>Vous pouvez toujours afficher vos devises par défaut  [!DNL Commerce] et Amazon lorsque d’autres devises sont disponibles. Si votre devise par défaut [!DNL Commerce] correspond à votre devise Amazon par défaut, laissez la fonction Conversion de devise désactivée.<br><br>Par exemple, si votre devise  [!DNL Commerce] par défaut est CAD (dollars canadiens) et votre devise par défaut Amazon est USD, vous devez activer Conversion de devise et choisir la valeur CAD du taux de conversion en USD. Les options présentées sont basées sur les conversions de devise [!DNL Commerce] intégrées. Si vous ne voyez pas l’option que vous recherchez, [configurez la devise dans [!DNL Commerce]](https://docs.magento.com/user-guide/stores/currency-configuration.html){target=&quot;_blank&quot;}. |
